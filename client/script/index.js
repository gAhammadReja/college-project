...
<div class="profile">
<img src='http://localhost:3000/photos/${photo}' alt="Welcome To ${college}" class="profileImage">
    <div class="mapouter">
...
<h2 class="btn-cancel" onclick="filterCollege('http://localhost:3000/filter')"> BACK </h2>
    </div>
    `;
}
function toggle(id){
...
document.getElementById('districtFilter').addEventListener("click",()=>{
    ...
    let url = `http://localhost:3000/filter?district_name=${filterName}`;
    filterCollege(url);
})
document.getElementById('streamFilter').addEventListener("click",()=>{
    ...
    alert(filterName)
    let url = `http://localhost:3000/filter?department=${filterName}`;
    filterCollege(url);
})
...
window.onload(filterCollege(`http://localhost:3000/filter`))